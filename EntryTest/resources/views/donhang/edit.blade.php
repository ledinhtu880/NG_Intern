                                ...
                                <div class="d-flex justify-content-between gap-5">
                                    ...
                                    <div class="form-group flex-grow-1">
                                        ...
                                        @if ($errors->has('Ngay_DatHang'))
                                            <input type="date" name="Ngay_DatHang" id="Ngay_DatHang"
                                                class="form-control is-invalid" placeholder="Nhập ngày đặt hàng"
                                                value="{{ old('Ngay_DatHang') }}">
                                            <span class="text-danger">
                                    ...
                                    @foreach ($data as $each)
                                        <tr class="js-row">
                                            <form class="formDetails" method="post">
                                                ...
                                                <td data-id="category" class="text-center" style="width: 125px">
                                                    {{ $each->Ten_LoaiHang }}</td>
                                                <td style="width: 400px">
                                                    <select name="FK_Id_MatHang" class="form-select">
                                                        @foreach ($items as $item)
                                                            @if ($item->Id_MatHang === $each->FK_Id_MatHang)
                                                                <option value="{{ $item->Id_MatHang }}" selected>
                                                                    {{ $item->Ten_MatHang }}</option>
                                                            @else
                                                                <option value="{{ $item->Id_MatHang }}">
                                                                    {{ $item->Ten_MatHang }}</option>
                                                            @endif
                                                        @endforeach
                                                    </select>

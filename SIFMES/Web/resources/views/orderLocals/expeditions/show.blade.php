...
@section('title', 'Xem chi tiết đơn giao hàng')
@section('content')
    ...
    <div class="row g-0 px-3">
        <h4 class="dashboard-title rounded-3 h4 fw-bold text-white m-0">Xem chi tiết đơn giao hàng</h4>
    </div>
    <div class="row g-0 p-3">
        <div class="col-md-12">
            <div class="card">
            ...
            <div class="card">
                            ...
                            @foreach ($data as $each)
                                <tr class="align-middle">
                                    ...
                                    <td class="text-center">
                                        {{ number_format(
                                            $each->Price_Container,
                                            $each->Price_Container == (int) $each->Price_Container ? 0 : 2,
                                            ',',
                                            '.',
                                        ) }}
                                    <td class="text-center">
                                        {{ number_format(
                                            $each->Price_Container * $each->Count_Container,
                                            $each->Price_Container == (int) $each->Price_Container ? 0 : 2,
                                            ',',
                                            '.',
                                        ) . ' VNĐ' }}
                ...
                <div class="card-footer pt-0 border-0 bg-transparent">
                    <div class="d-flex justify-content-end align-items-center"><a
                            href="{{ route('orderLocals.expeditions.index') }}" class="btn btn-secondary">Quay lại</a>
                    </div>
                </div>
            </div>
        </div>

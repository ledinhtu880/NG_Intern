    ...
    <div class="col-md-12">
      <nav aria-label="breadcrumb">
        <ol class="breadcrumb breadcrumb-color px-2 py-3 rounded">
          ...
          <li class="breadcrumb-item active">
            <a class="text-decoration-none" href="/orders/simples/createSimple?id={{ $_GET['id'] }}">Thêm đơn thùng
              hàng</a>
          </li>
        ...
        if (response == 0) {
          ...
          for (var i = 1; i <= col * row; i++) {
            if (details[i - 1].FK_Id_StateCell == "0") {
              ...
              $('#cell' + i).attr("data-status", 0)
              $('#cell' + i).append(
                $(`<p class="small" data-id="${details[i - 1].FK_Id_ContentSimple}"
                      id="simple-${details[i - 1].FK_Id_ContentSimple}">Thùng hàng số ${details[i - 1].FK_Id_ContentSimple}
                      ...
                      <table class="table table-striped m-0">
                        <thead>
                          <tr>
                            ...
                            <th class="text-center" scope="col">Số lượng khả dụng</th>
                    ...
                    <div class="modal-footer" style="height: 100px;">
                        <div class="d-flex align-items-stretch justify-content-between w-100">
                          ...
                          <div class="d-flex gap-2">
                            ...
                            <button type="button" class="btn btn-primary-color btnTake" data-id="${details[i - 1].FK_Id_ContentSimple}">Lấy thùng hàng</button>
                          </div>
                        </div>
                    </div>
                  </div>
    ...
    $("#saveBtn").on("click", function () {
      ...
      cellElements.each(function () {
        if ($(this).attr("istake")) {
          ...
          dataArr.push({ id: id, Count: Count });
        }
      });

...
class DispatchController extends Controller
{
  ...
  public function showOrderByRawType(Request $request)
  {
  ...
  public function showOrderByItemType(Request $request)
  {
    $itemType = $request->input('itemType');
    if ($itemType == 0) {
      $psl = DB::table('ProductionStationLine')
        ->select('Id_ProdStationLine', 'Name_ProdStationLine', 'Description')
        ->join('OrderType', 'FK_Id_OrderType', '=', 'Id_OrderType')
        ->where('Id_ProdStationLine', 3)->get();
      $orderlocals = DB::table('DetailContentSimpleOrderLocal as DL')
        ->join('OrderLocal as O', 'O.Id_OrderLocal', '=', 'DL.FK_Id_OrderLocal')
        ->where('O.MakeOrPackOrExpedition', 2)
        ->where('O.SimpleOrPack', $itemType)
        ->select('O.*')->distinct()
        ->whereNotIn('Id_OrderLocal', function ($query) {
          $query->select('FK_Id_OrderLocal')
            ->from('DispatcherOrder');
      ...
      $psl = DB::table('ProductionStationLine')
        ->select('Id_ProdStationLine', 'Name_ProdStationLine', 'Description')
        ->join('OrderType', 'FK_Id_OrderType', '=', 'Id_OrderType')
        ->where('Id_ProdStationLine', 4)->get();
      $orderlocals = DB::table('DetailContentPackOrderLocal as DL')
        ->join('OrderLocal as O', 'O.Id_OrderLocal', '=', 'DL.FK_Id_OrderLocal')
        ->where('O.MakeOrPackOrExpedition', 2)
        ->where('O.SimpleOrPack', $itemType)
        ->select('O.*')->distinct()
      ...
      if ($status == 0) {
        ...
        if ($SimpleOrPack == 0) {
          ...
          $contentSimples = DB::table('DetailContentSimpleOfPack')
            ...
            ->join('ContentSimple as cs', 'FK_Id_ContentSimple', '=', 'cs.Id_ContentSimple')

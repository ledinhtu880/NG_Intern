...
class TrackingController extends Controller
{
  ...
  public function ShowOrderByDate(Request $request)
  {
    if ($request->ajax()) {
      ...
      $data = DB::table('Order')
        ...
        ->select('Id_Order', 'c.Name_Customer', 'Date_Order', 'Date_Dilivery', 'SimpleOrPack')
        ->selectRaw("CASE WHEN SimpleOrPack = 0 THEN COUNT(DISTINCT cs.Id_SimpleContent) ELSE COUNT(DISTINCT cp.Id_PackContent) END AS TotalCount")
        ->groupBy('Id_Order', 'c.Name_Customer', 'Date_Order', 'Date_Dilivery', 'SimpleOrPack')
        ->get();
      ...
      foreach ($data as $each) {
        ...
        if ($each->SimpleOrPack == 0) {
          $simples = ContentSimple::where('FK_Id_Order', $each->Id_Order)->pluck('Id_SimpleContent')->toArray();
          ...
          $packs = ContentPack::where('FK_Id_Order', $each->Id_Order)->pluck('Id_PackContent')->toArray();
  ...
  public function showPacks(string $id)
  {
    ...
    $data = DB::table('ContentPack as cp')
      ->leftJoin('DetailContentSimpleOfPack as dcsp', 'cp.Id_PackContent', '=', 'dcsp.FK_Id_PackContent')
      ->select('cp.Id_PackContent', 'cp.Count_Pack', 'cp.Price_Pack', DB::raw('COUNT(DISTINCT dcsp.FK_Id_SimpleContent) as TotalSimple'))
      ->groupBy('cp.Id_PackContent', 'cp.Count_Pack', 'cp.Price_Pack')
      ->get();
    ...
    foreach ($data as $each) {
      ...
      $simples = DB::table('ContentSimple')
        ->join('DetailContentSimpleOfPack', 'ContentSimple.Id_SimpleContent', '=', 'DetailContentSimpleOfPack.FK_Id_SimpleContent')
        ->where('FK_Id_PackContent', $each->Id_PackContent)
        ->pluck('Id_SimpleContent')->toArray();
  ...
  public function showSimples(string $id)
  {
    ...
    $data = DB::table('ContentSimple as cs')
      ...
      ->groupBy('cs.Id_SimpleContent', 'rm.Name_RawMaterial', 'rm.Unit', 'ct.Name_ContainerType', 'cs.Count_Container', 'cs.Price_Container')
      ->select('cs.Id_SimpleContent', 'rm.Name_RawMaterial', 'rm.Unit', 'ct.Name_ContainerType', 'cs.Count_Container', 'cs.Price_Container')
      ->get();
    foreach ($data as $each) {
      $result = DB::table('ContentSimple as cs')
        ->join('DetailContentSimpleOrderLocal as dcso', 'cs.Id_SimpleContent', '=', 'dcso.FK_Id_ContentSimple')
        ...
        ->join(DB::raw('(SELECT MAX(FK_Id_Station) AS FK_Id_Station, FK_Id_ContentSimple FROM ProcessContentSimple GROUP BY FK_Id_ContentSimple) pcs'), 'cs.Id_SimpleContent', '=', 'pcs.FK_Id_ContentSimple')
        ->where('cs.Id_SimpleContent', '=', $each->Id_SimpleContent)
      ...
      if ($result != null) {
        ...
        $countComplete = DB::table('ProcessContentSimple')
          ->where('FK_Id_ContentSimple', $each->Id_SimpleContent)
          ->where('FK_Id_State', 2)
          ->whereNotNull('Data_Fin')
          ->count();
  ...
  public function showDetailsSimple(string $id)
  {
    ...
    $result = DB::table('ProcessContentSimple')
      ->select('FK_Id_Station', 'Data_Fin', DB::raw("CONCAT(
        ...
        (DATEDIFF(second, Data_Start, Data_Fin) % 60), N' giây'
    ) as elapsedTime"))

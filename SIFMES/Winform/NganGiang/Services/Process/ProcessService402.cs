...
namespace NganGiang.Services.Process
{
    internal class ProcessService402
    {
        public DataTable getProcessContentSimple()
        {
            string query =
                @"SELECT FK_Id_OrderLocal AS [Mã đơn hàng], 
                    ...
                    Id_ContentSimple AS [Mã thùng hàng],  Name_RawMaterial AS [Tên nguyên liệu thô],
                    RawMaterial.Count AS [Số lượng nguyên liệu tồn], Count_RawMaterial * Count_Container AS [Số lượng nguyên liệu cần], 
                    Unit AS [Đơn vị], Name_State AS [Trạng thái], CONVERT(date, ProcessContentSimple.Date_Start) AS [Ngày bắt đầu], 
                    ...
                    WHERE FK_Id_Station = 402 AND FK_Id_State != 2
                    GROUP BY FK_Id_OrderLocal, SimpleOrPack, Id_ContentSimple, Name_RawMaterial, RawMaterial.Count, Unit, Name_State, 
                    Count_RawMaterial * Count_Container, CONVERT(date, ProcessContentSimple.Date_Start), FK_Id_RawMaterial, Count_Container, FK_Id_ContainerType";
            DataTable dt = DataProvider.Instance.ExecuteQuery(query);
            return dt;
        }

        public bool UpdateContentSimple(int Id_ContentSimple, out string message)
        {
            message = "";
        ...
        public bool UpdateProcessContentSimple(int Id_ContentSimple, out string message)
        {
            message = "";
            ...
            processContentSimple.Date_Fin = System.DateTime.Now.ToString("yyyy-MM-dd HH:mm:ss");

            string query = $"UPDATE ProcessContentSimple SET FK_Id_State = {processContentSimple.FK_Id_State}, Date_Fin = '{processContentSimple.Date_Fin}' WHERE FK_Id_ContentSimple = {processContentSimple.FK_Id_ContentSimple} AND FK_Id_Station = 402";
        ...
        public bool InsertProcessContentSimple(int Id_ContentSimple, out string message)
        {
            message = "";
            ...
            processContentSimple.Date_Start = System.DateTime.Now.ToString("yyyy-MM-dd HH:mm:ss");
        ...
        public bool UpdateRawMaterial(int Id_ContentSimple, out string message)
        {
            message = "";
